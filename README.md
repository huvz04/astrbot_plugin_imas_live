...
`/imasflight` 是可选的上海（PVG/SHA）—东京（NRT/HND）往返行程规划入口。它会先列出官网已核验的实际场次，管理员必须明确选一个场次 ID；`all` 也必须显式写出。新计划按所选场次的日本当地日期生成「前 2 / 前 1 天抵达、后 1 / 后 2 天返程」候选，偏好直飞，且默认永久暂停。当前版本不包含报价 API 调用、密钥、自动查询、自动提醒或任何额度消费；`flight_provider` 与 `flight_target_price_cny` 仅供把价格规则记录进新计划，待报价数据源实现并由管理员显式启用后才可激活。
...
| `flight_provider` | 空 | 可选报价数据源标识；当前仅记录进暂停计划，绝不发请求。 |
| `flight_target_price_cny` | `0` | 往返心理价；`0` 代表未设置，当前不会提醒。 |

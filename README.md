...
/imasflight <活动编号>
/imasflight plan <活动编号> <场次ID|all>
/imasflight route <东京到达日YYYY-MM-DD> <东京返程日YYYY-MM-DD>
/imasflight subscribe <出发地> <目的地> <出发日YYYY-MM-DD> <返程日YYYY-MM-DD>
/imasflight unsubscribe <订阅ID>
/imasflight list
/imasflight price <计划ID> <人民币心理价>
/imasflight baggage <计划ID> <none|checked>
/imasflight enable|disable <计划ID>
/imasflight check <计划ID>
```
...
官网目录偶有缺失已核验的官方活动页；插件仅为这类明确维护的受控 URL 补充发现，不扫描或猜测 URL。现有安装发现该类来源时保留基线，不把历史抽选自动公告到群。
...
`/imasflight` 是可选的上海（PVG/SHA）—东京（NRT/HND）往返监测入口。它先列出官方场次，管理员明确输入一个场次 ID、逗号分隔的多个 ID，或显式输入 `all`。新计划按所选场次的日本当地日期生成「前 2 / 前 1 天抵达、后 1 / 后 2 天返程」候选，偏好直飞。管理员需逐计划设置心理价，然后配置 SerpApi 密钥与正数月度预算，再执行 `enable`；默认全停用且月度请求预算为 0。机票订阅不继承 LIVE 或抽票订阅。

普通用户可在想接收通知的会话执行 `/imasflight subscribe 上海 东京 2027-07-23 2027-07-26`，自行指定出发地、目的地、出发日和返程日。地点支持三字机场代码（如 `PVG`、`NRT`，也可用逗号指定最多四个机场）及上海、东京、北京、大阪四个城市别名；订阅只监测固定日期的往返经济舱完整报价，含直飞和中转。首次成功查价仅建立基线，此后在已展开的候选中最低往返价每次上涨或下降都会通知，无需心理价门槛；只能停止自己在当前会话创建的订阅，每人每会话最多同时启用三条。出发日过后自动停止。订阅由用户创建，但须先由管理员配置 SerpApi 数据源和正数月度请求预算；预算默认 0，未配置时会明确拒绝创建且不会查询。检测间隔默认 30 分钟，结果最多共享缓存 30 分钟，额度用尽时不会继续请求；每条航线每月在默认最多展开 2 条返程的设置下，理论上最多可消耗约 4320 次请求，实际取决于候选数量和共享缓存。价格变化指查询所返回的最低候选报价变化，不保证覆盖所有航空公司或票价。

监测每轮按日期组合轮换，用 SerpApi 的机场列表一次覆盖上海双机场和东京双机场，首次响应的去程 `departure_token` 还需按限额展开返程。只有去回两段机场、日期和直飞条件都核验后，才可进入心理价提醒。返程未展开的报价不能充当完整往返提醒；Aviasales 适配器尚未接入，不会查询或推送。缺少托运行李资料时，如任务要求 `checked`，该报价不会提醒。页面 `pages/flight/` 在 AstrBot 插件详情中展示任务、缓存报价、查询错误和请求预算；图片后附来源搜索链接，价格与可售性仍以来源结果页为准。多个会话的相同查询共享缓存，投递按完整会话 ID 独立去重；发送失败保留待投递，活动改期或场馆/来源变化自动暂停旧计划。

普通日期监测可用 `/imasflight route` 指定东京抵达日和返程日。
...
`/imaslive enable|disable` 与 `/imasticket enable|disable` 是 AstrBot 原生管理员命令，权限由 AstrBot 的管理员配置和指令管理统一决定；插件不维护 `admin_ids`，也不把消息平台的群管理员自动当作 AstrBot 管理员。普通用户仍可使用 LIVE、月份、`next`、抽票查询和 `get`。AstrBot 4.27.3 的指令组会强制把裸组名渲染为帮助，不能同时承载 `/imaslive` 这类默认查询；为了保留既有语法并消除重复注册告警，插件将 `imaslive`、`imaslive next`、`imaslive enable` 等注册为独立的原生命令。它们可在 WebUI 的“指令管理”中分别查看、启停或重命名。私聊不会设置订阅。`/imaslive enable` 同时控制 LIVE 开演前一小时提醒和新增现场抽选公告；`/imasticket enable` 只控制抽票截止前 24 小时与 1 小时提醒。两种订阅分别按完整 UMO 保存，互不影响。已订阅群在节点后短暂的发送或调度故障会在默认 20 分钟恢复窗口内重试；不会跨到下一节点、不会在截止后补发，重新开启订阅也不会回放旧提醒。
...
| `flight_providers` | `[]` | 选择 `serpapi`；`aviasales` 目前仅预留，不发起查询。 |
| `flight_serpapi_key` | 空 | 服务器端数据源密钥；不返回网页或日志。 |
| `flight_target_price_cny` | `0` | 新计划的默认往返心理价；也可用 `imasflight price` 逐计划设置。 |
| `flight_serpapi_monthly_budget` | `0` | 每月最多尝试的 SerpApi 请求数，包含返程展开；0 禁止请求。 |
| `flight_max_queries_per_cycle` | `1` | 每轮最多轮换查询的机场/日期组合。 |
| `flight_max_return_expansions` | `2` | 每个去程搜索最多展开的返程候选。 |
| `flight_interval_minutes` | `30` | 同一计划的自动检查间隔，最低 30 分钟。 |

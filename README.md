...
提醒检查每 30 秒独立运行，不等待较慢的官网同步。仅在轮次为 `onsite + lottery`、起售已开始、截止时间明确、数据未陈旧，且满足 `deadline - reminder_before_minutes <= now < deadline` 时触发。`/imasticket` 的 24 小时红色仅为查询视觉提示，不会新增自动推送。

...
`/imaslive` 会生成一张完整 PNG 长图，只展示从今天 00:00 起未来 30 个自然日内的演出。`/imaslive 1—12` 查询最近一次对应月份的完整自然月：月份不小于当前月时使用本年，小于当前月时使用下一年；例如 2026 年 9 月查 `10` 是 2026 年 10 月，查 `1` 是 2027 年 1 月，查 `9` 是完整的 2026 年 9 月（包括已经过去的日期）。

`/imasticket` 单独展示近期现场抽选：所有仍可申请的轮次都会保留，即使截止超过未来 30 天；另包含未来 30 天内即将开始的轮次。先到先得、转售、配信、物販、当落和付款均不进入该图。
...
- LIVE 图不混入抽票卡。抽票图中，24 小时内截止为淡红色“24小时内截止”，正在抽选为淡绿色“正在抽选”，即将开始、开始时间缺失和陈旧缓存均为灰色待核验状态。
- 抽票卡标出活动、轮次、含年份的显示时区与 JST 开始/截止时间；已知的演出日期和场地会附在卡内。官方申请链接以可点击文本随图片返回。
- 没有记录时仍会生成清晰的空状态图片。
...
提醒检查每 5 分钟独立运行，不等待较慢的官网同步。官网数据一旦进入本地缓存，之后即使没有新的远程请求，轮次也会在进入 `deadline - reminder_before_minutes <= now < deadline` 窗口时触发。仅限 `onsite + lottery`、起售已开始、截止时间明确且数据未陈旧的轮次；`/imasticket` 的 24 小时红色仅为查询视觉提示，不会新增自动推送。
